...
export default function Analyze() {
    ...
    const parseReceiptText = (rawText: string): ParsedItem[] => {
        const lines = rawText.replace(/[※\*＊%]/g, '').replace(/[,，]/g, '').replace(/[¥\\￥]/g, '').split('\n').map(l => l.trim()).filter(l => l.length > 0);
        const parsedItems: ParsedItem[] = [];
        const ignoreWords = ["合計", "お預", "お釣", "釣銭", "レジ", "電話", "住所", "店舗", "小計", "税", "割引", "ポイント", "領収", "担当", "日付", "商品数", "対象", "クレジット", "売上票", "控え", "番号", "支払", "ID", "取"];

        for (let i = 0; i < lines.length - 1; i++) {
            if (ignoreWords.some(w => lines[i].includes(w)) || /^[\d\s]+$/.test(lines[i]) || lines[i].length < 2) continue;
            let name = "";
            if (/^\d+$/.test(lines[i + 1])) { name = lines[i].normalize("NFKC"); i++; }
            else { const m = lines[i].match(/^(.*?)\s+(\d+)$/); if (m) name = m[1].trim().normalize("NFKC"); }

            if (name) {
                // ⭐️ 「レシート表示名(receipt_name)」と一致するかチェックする（旧データ用に name もチェック）
                const p = products.find(prod => name.includes(prod.receipt_name) || name.includes(prod.name));
                if (p) {
                    // 見つかったら「本当の商品名(name)」と「マスタの単価」をリストに入れる！
                    parsedItems.push({ name: p.name, price: p.price, qty: 1 });
                }
            }
        }
        const agg: Record<string, ParsedItem> = {};
        parsedItems.forEach(item => { const k = `${item.name}_${item.price}`; if (agg[k]) agg[k].qty += 1; else agg[k] = { ...item }; });
