    ...
    return (
        <div className="p-4 max-w-4xl mx-auto h-[80vh] flex flex-col">
            <div className="flex justify-between mb-2">
                ...
                <div><button onClick={() => setMyRole('販売')} className={`px-4 ${myRole === '販売' ? 'bg-[#D4A96A] text-white' : 'bg-white'}`}>🛒 販売</button><button onClick={() => setMyRole('製造')} className={`px-4 ${myRole === '製造' ? 'bg-[#8B5E3C] text-white' : 'bg-white'}`}>🥖 製造</button></div>
            </div>
            ...
            <div className="flex-1 bg-white p-4 overflow-y-auto rounded-t-xl border">
                {messages.map(m => (<div key={m.id} className={`flex flex-col ${m.sender === myRole ? 'items-end' : 'items-start'} mb-4`}><div className={`px-4 py-3 rounded-2xl max-w-[80%] whitespace-pre-wrap ${m.sender === myRole ? 'bg-[#D4A96A] text-white rounded-br-none' : 'bg-gray-100 rounded-bl-none'}`}>{m.content}</div></div>))}
                <div ref={endRef} />
            </div>
